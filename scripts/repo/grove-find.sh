...
#   source /path/to/your-project/scripts/repo/grove-find.sh
#
...
# Get the project root directory (script is in scripts/repo/, so go up 2 levels)
...
if [ -n "${BASH_SOURCE[0]:-}" ]; then
    ...
    GROVE_ROOT="$(cd "$(dirname "$0")/../.." 2>/dev/null && pwd)" || GROVE_ROOT="$(pwd)"
fi
